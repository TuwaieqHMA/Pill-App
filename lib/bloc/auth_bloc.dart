...
class AuthBloc extends Bloc<AuthEvent, AuthState> {
    ...
    if (event.name.trim().isNotEmpty &&
      ...
      if (int.parse(event.age) >= 7 && int.parse(event.age) < 105) {
        try {
          ...
          emit(AuthSignUpState(msg: "تم إنشاء الحساب بنجاح"));
  ...
  FutureOr<void> signout(SignOutEvent event, Emitter<AuthState> emit) async {
    ...
    try {
      ...
      locator.currentUser = SaedUser(name: "بك", email: "someone@hotmail.com", password: "123456", age: 24);
  ...
  FutureOr<void> sendOtp(SendOtpEvent event, Emitter<AuthState> emit) async {
    ...
    if (event.email.trim().isNotEmpty) {
      try {
        ...
        emit(AuthEmailVerifiedState(msg: "تم إرسال رمز التحقق إلى بريدك الإلكتروني"));
      } catch (e) {
  ...
  FutureOr<void> getUserInfo(GetUserInfoEvent event, Emitter<AuthState> emit) async{
    emit(AuthLoadingState());
  ...
  FutureOr<void> activateEditMode(ActivateEditModeEvent event, Emitter<AuthState> emit) {
    emit(IsEditingProfileState(isEditing: true));
  }

  FutureOr<void> deactivateEditMode(DeactivateEditModeEvent event, Emitter<AuthState> emit) async{
    emit(IsEditingProfileState(isEditing: false));
    await getUserInfo(GetUserInfoEvent(), emit);
  }

  FutureOr<void> editProfile(EditProfileEvent event, Emitter<AuthState> emit) async{
    emit(AuthLoadingState());

    if(event.name.trim().isNotEmpty && event.age.trim().isNotEmpty && event.password.trim().isNotEmpty){
      if (int.parse(event.age) > 7 && int.parse(event.age) < 120){
        try {
      await DBService().editUserInfo(name: event.name, age: int.parse(event.age), password: event.password);
      final SaedUser user = await DBService().getCurrentUserInfo();
      locator.currentUser = user;
